...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "✨ <b>Добро пожаловать в наш магазин hukate star!</b> ✨\n\n"
...
async def process_buy_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "🔢 Введите количество звезд, которое хотите приобрести, больше 50! (например: <code>50</code>):",
...
async def process_screenshot(message: Message, state: FSMContext, bot: Bot):
    ...
    admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ все чотка, принять", callback_data=f"approve_{user.id}_{stars}"),
            InlineKeyboardButton(text="❌ все плоха, откланить", callback_data=f"decline_{user.id}")
        ]
    ])
...
async def process_calc_result(message: Message, state: FSMContext):
    ...
    amount = int(message.text)
    if amount <= 50:
        await message.answer("⚠️ Количество должно быть больше 50.", reply_markup=get_back_to_menu_keyboard())
        return
...
async def process_support_menu(callback: CallbackQuery):
    text = (
        ...
        f"👤 <b>Администратор / Поддержка:</b> @{hukate}\n\n"
        f"📢 <b>Наши каналы и проекты:</b>\n"
        f"• <a href='https://t.me/+dZZTSZjwwyszMjlK'>Наш главный канал</a>\n"
        f"• <a href='https://t.me/hukateACC'>Отзывы клиентов</a>\n\n"
